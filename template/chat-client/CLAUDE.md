...
| Already built | Where |
|---|---|
| Chat UI with message thread | `@src/App.tsx` |
| `POST /a2a` call with `contextId` multi-turn | `@src/App.tsx` — `sendMessage()` function |
| Tool event trace panel | Side panel in `@src/App.tsx`, reads `metadata.tool_events` |
| UCP checkout card (payment flow) | Checkout component in `@src/App.tsx` |
| WebSocket trace (`/ws/trace`) listener | Connected in `@src/App.tsx` |
