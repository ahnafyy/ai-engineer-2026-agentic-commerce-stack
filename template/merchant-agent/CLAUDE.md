...
2. `@main.py` — `SYSTEM_PROMPT`, `AGENT_CARD`, `UCP_PROFILE`, `OPENAI_TOOLS`
...
| `@main.py` | `SYSTEM_PROMPT`, `AGENT_CARD`, `UCP_PROFILE`, `OPENAI_TOOLS` |
...
2. Add the matching entry to `OPENAI_TOOLS` in `@main.py` (the OpenAI function-calling format)
3. Add a dispatch case in the `handle_tool_call` function in `@main.py`
...
**`@main.py`** — add to `handle_tool_call`:
```python
